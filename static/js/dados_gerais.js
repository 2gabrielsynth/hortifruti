// static/js/dados_gerais.js
document.addEventListener('DOMContentLoaded', function() {
    console.log('Dados Gerais - Inicializando...');
    ...
    if (ctx) {
        ctx.parentElement.innerHTML = `
            <div style="text-align: center; padding: 3rem; color: var(--text-tertiary);">
                ...
                <small>Mostrando variações individuais das frutas</small>
            </div>
        `;
    }
}
...
function setupEventListeners() {
    ...
    if (sortTotalBtn) sortNameBtn.addEventListener('click', () => sortTable('total'));
    if (sortVarBtn) sortNameBtn.addEventListener('click', () => sortTable('var'));
...
function exportData() {
    try {
        ...
        link.setAttribute('download', 'dados_gerais_frutas.csv');
...
function downloadChart() {
    ...
    const link = document.createElement('a');
    link.download = 'grafico_performance.png';
