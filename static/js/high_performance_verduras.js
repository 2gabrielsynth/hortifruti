// high_performance_verduras.js - VERSÃO SIMPLIFICADA
const fmtMoney = v => Number(v || 0).toLocaleString('pt-BR', {style:'currency',currency:'BRL'});
const fmtPct = v => (v>0?'+':'') + Number(v || 0).toFixed(2) + '%';

// Puxa dados do endpoint CORRETO para verduras
fetch('/api/verduras/high')  // ← MUDANÇA AQUI
  .then(res => {
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return res.json();
  })
  .then(dados => {
    console.log('Dados de verduras recebidos:', dados);
  ...
  .catch(err => {
    console.error('Erro ao carregar dados de verduras:', err);
    ...
    erroDiv.style.cssText = `
      ...
      text-align: center;
    `;
    erroDiv.textContent = `Erro ao carregar dados de verduras: ${err.message}`;
...
function exportarDados(dados) {
  const conteudoCSV = "Verdura;Total 2025;Média Mensal;Crescimento(%)\n" +  // ← MUDANÇA AQUI
    dados.map(verdura => `"${verdura.nome}";${verdura.total};${verdura.media};${verdura.var}`).join("\n");
  ...
  link.href = url;
  link.download = 'high_performance_verduras.csv';  // ← MUDANÇA AQUI
  link.click();
