...
async function buscarDadosTop5() {
    try {
        const resposta = await fetch('/api/verduras/top5');
...
async function buscarDadosComparativos() {
    try {
        const resposta = await fetch('/api/verduras/chart-data?chart_type=top5_comparison');
...
function popularTabela() {
    ...
    dadosTop5.forEach((verdura, indice) => {
        const linha = document.createElement('tr');
        
        const variacao = Number(verdura.variacao);
        ...
        linha.innerHTML = `
            ...
            <td><strong>${verdura.nome}</strong></td>
            <td>${formatarMoeda(verdura.total)}</td>
            <td>${formatarMoeda(verdura.media)}</td>
...
function exportarDados() {
    const conteudoCSV = "Verdura;Total 2025;Média Mensal;Variação(%)\n" +
        dadosTop5.map(verdura => `"${verdura.nome}";${verdura.total};${verdura.media};${verdura.variacao}`).join("\n");
    ...
    link.href = url;
    link.download = 'top5_verduras.csv';
    link.click();
...
function baixarGrafico() {
    const link = document.createElement('a');
    link.download = 'grafico_top5_verduras.png';
...
function inicializarModalKPI() {
            ...
            conteudo: function() {
                ...
                return `
                    <div class="modal-kpi-content">
                        ...
                        <p>O faturamento total das 5 verduras líderes soma <strong>${formatarMoeda(faturamentoTotal)}</strong> em 2025.</p>
                        <div class="modal-stats">
                            <div class="stat-item">
            ...
            conteudo: function() {
                ...
                return `
                    <div class="modal-kpi-content">
                        ...
                        <p>A média geral de faturamento mensal entre as 5 líderes é de <strong>${formatarMoeda(mediaGeral)}</strong> por verdura.</p>
                        <div class="modal-stats">
                            <div class="stat-item">
    ...
    function calcularParticipacao(totalVerdura) {
        const totalGeral = dadosTop5.reduce((soma, item) => soma + item.total, 0);
        return ((totalVerdura / totalGeral) * 100).toFixed(1);
    }
