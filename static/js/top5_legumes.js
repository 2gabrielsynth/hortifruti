...
async function buscarDadosTop5() {
    try {
        const resposta = await fetch('/api/legumes/top5');
...
async function buscarDadosComparativos() {
    try {
        const resposta = await fetch('/api/legumes/chart-data?chart_type=top5_comparison');
...
function popularTabela() {
    ...
    dadosTop5.forEach((legume, indice) => {
        const linha = document.createElement('tr');
        
        const variacao = Number(legume.variacao);
        ...
        linha.innerHTML = `
            ...
            <td><strong>${legume.nome}</strong></td>
            <td>${formatarMoeda(legume.total)}</td>
            <td>${formatarMoeda(legume.media)}</td>
...
function exportarDados() {
    const conteudoCSV = "Legume;Total 2025;Média Mensal;Variação(%)\n" +
        dadosTop5.map(legume => `"${legume.nome}";${legume.total};${legume.media};${legume.variacao}`).join("\n");
    ...
    link.href = url;
    link.download = 'top5_legumes.csv';
    link.click();
...
function baixarGrafico() {
    const link = document.createElement('a');
    link.download = 'grafico_top5_legumes.png';
...
function inicializarModalKPI() {
            ...
            conteudo: function() {
                ...
                return `
                    <div class="modal-kpi-content">
                        <h4>${lider.nome} - Líder Absoluto</h4>
                        <p>O ${lider.nome} mantém a primeira posição no ranking com faturamento médio mensal de <strong>${formatarMoeda(lider.media)}</strong>.</p>
            ...
            conteudo: function() {
                ...
                return `
                    <div class="modal-kpi-content">
                        ...
                        <p>O faturamento total dos 5 legumes líderes soma <strong>${formatarMoeda(faturamentoTotal)}</strong> em 2025.</p>
                        <div class="modal-stats">
                            <div class="stat-item">
            ...
            conteudo: function() {
                ...
                return `
                    <div class="modal-kpi-content">
                        ...
                        <p>O ${maiorCrescimento.nome} registrou o maior crescimento entre os top performers, com <strong>${formatarPorcentagem(maiorCrescimento.variacao)}</strong> em 2025.</p>
                        <div class="modal-stats">
                            <div class="stat-item">
                                <span class="stat-label">Faturamento 2025:</span>
            ...
            conteudo: function() {
                ...
                return `
                    <div class="modal-kpi-content">
                        ...
                        <p>A média geral de faturamento mensal entre os 5 líderes é de <strong>${formatarMoeda(mediaGeral)}</strong> por legume.</p>
                        <div class="modal-stats">
                            <div class="stat-item">
    ...
    function calcularParticipacao(totalLegume) {
        const totalGeral = dadosTop5.reduce((soma, item) => soma + item.total, 0);
        return ((totalLegume / totalGeral) * 100).toFixed(1);
    }
